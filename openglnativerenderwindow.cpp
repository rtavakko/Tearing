...
LRESULT OpenGLNativeRenderWindow::WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        //Store the OpenGL context handle for the current render window object
        renderWindow = reinterpret_cast<OpenGLNativeRenderWindow*>(GetWindowLongPtrW(hwnd,0));
        if(renderWindow)
            if(renderWindow->isVisible())
                renderWindow->setOpenGLContextHandle(hglrc);
        break;
        case WM_DESTROY:
        ...
        if(renderWindow)
        {
            if(renderWindow->isVisible())
            {
                if(renderWindow->makeContextCurrentNative())
                    renderWindow->renderFrame();
                renderWindow->swapSurfaceBuffersNative();
                renderWindow->doneContextCurrent();
            }
        }
...
void OpenGLNativeRenderWindow::setOpenGLContextHandle(HGLRC handle)
{
    hglrc = handle;
}
